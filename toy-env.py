"""Making sure we are running right version of python"""
import sys
...
def cli(agent):
	...
	while cmd not in ("exit", "quit"):
		...
		cmd = raw_input("soar> ")
#""" Output monitor"""
#def output_event_handler(obj, agent_name, attribute, wme):
#    if wme.IsJustAdded(): #attribute == "result" and 
#        res = float(wme.GetValueAsString())#(int)(wme.GetValueAsString())
#        print("output::::::::: ",res)

from random import *
class ToyEnv(object):
    """
    A very simple 'environment': sensors retur two random numbers and expects a single number as actuation.
    """
    def __init__(self):
...
#    #Create a monitor for outputs
#    agent.AddOutputHandler("test", output_event_handler, None)

    #Load soar sources
    agent.ExecuteCommandLine("source toy-env.soar")
#    print(agent.ExecuteCommandLine("source wj.soar"))
    ...
    # 1) sense the environment
        ...
        result=0
        run_result=agent.RunSelf(2)    #Run agent for one step
